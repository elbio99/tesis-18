...
\end{enumerate}
La imagen anterior \ref{Fig: proposal} se obtuvo 53.815 regiones por el método \textit{Selective Search} de selección de candidatos. Como se mencionó en \ref{sub:nonmaximumsuppression} existen regiones solapadas que representan la misma información; esta información fue eliminada aplicando \ac{nms} 
...
Para este proceso se utilizó una red pre-entrenada \textbf{ResNet50} \ref{sub:arquitecturacnn} donde se obtuvo de la ante ultima capa el vector que va a representar la región.
...
En esta sección se desarrolla como se realizo el proceso de aprendizaje automático \ac{ml} con los datos obtenidos de la sección anterior \ref{sec:preparacion_de_datos}. Las imágenes finales utilizadas para realizar el proceso de entrenamiento de detalla en \ref{sec:datosutilizados}.

El datasets final se dividió en tres conjuntos train, test y validación. El total de la muestra de datos para la experimentación fue de 650.000 \textit{feature vector}.

\subsection{Hiper-parametros}\label{sub:hiperparametro}


Para obtener una predicción correcta se debe realizar la optimización de parámetros (ver: \ref{sub:hiperparametro}) que afectan a la clasificación. El método utilizado para poder realizar la optimización de estos parámetros fue \textit{grid search}\footnote{Fuente: https://goo.gl/koWr74}. El método grid-search busca por fuerza bruta los parámetros a optimizar quedándose con aquel que logre un mayor \textit{accuracy} en las evaluaciones; en cada iteración se utiliza \textit{cross-validation} con el valior de k=5 para el proceso de  entrenamiento.

Los parámetros a optimizar por \textit{grid-search} son:
\begin{enumerate}
	\item \textbf{Parámetro C}: Controla el costo de la mala clasificación, \textit{missclassification}, en los datos de entrenamiento. Con un valor de C pequeño hará que el optimizador busque un hiperplano de separación de mayor margen, incluso si ese hiperplano clasifica erróneamente más puntos. Para valores muy pequeños de C, debe obtener ejemplos mal clasificados, a menudo incluso si sus datos de entrenamiento son linealmente separables. Para valores grandes de C,  elegirá un hiperplano de menor margen si ese hiperplano hace un mejor trabajo de conseguir que todos los puntos de entrenamiento estén clasificados correctamente. El objetivo es encontrar el equilibrio entre "no demasiado estricto" y "no demasiado relajado". La validación cruzada  son buenas maneras de encontrar el mejor C.
...
El rango de parámetros que se uso para el entrenamiento y validación de datos fueron los siguientes: para valores de C se trabajo con (1000,100,10,1, 0.1,0.01,0.001,0.0001) y para valores de gamma se utilizaron (1,0,1,0,01,0,001,0,0001,0,00001)

\subsection{Resultados obtenidos}\label{sub:resultados}
Además de la optimización de los parámetros mencionados \ref{sub:hiperparametro}, se debe evaluar la performance del modelo entrenado, como se detallo en la sección \ref{sub:evaluación-modelo}, la validación de nuestro modelo serán basados en las medidas mencionada anteriormente.
...
Bandas & C & gamma & kernel & precision & recall & \begin{tabular}[c]{@{}c@{}}average\\ precision \end{tabular} & \begin{tabular}[c]{@{}c@{}}overlapping\\ mean\end{tabular} \\ \hline
...
Las tareas realizadas para evaluar la localización del objeto fue obtener la métrica \textit{overlapping mean}; en esta métrica comparamos los \textit{bounding boxs} obtenidos en el entrenamiento realizado y el \textit{ground thruth}, región de interés, que tenemos anotado. Este valor nos da una visión de cual es la exactitud de nuestro predictor en cuanto a la localización del objeto.

La siguiente imagen \ref{Fig: TP} se puede visualizar los \textit{bounding boxs} detectados, esto es el resultado de aplicar el modelo entrenado a una imagen satelital; en este caso la imagen utilizada pertenece a las bandas \textbf{M9-M7-M11}. Como se puede observar en la tabla \ref{tab:entrenam-result}, esta combinación de banda es una de las que mejor performance obtuvo al momento de evaluar el modelo en termino de localización, métrica overlapping mean, y precision.
...
En el siguiente cuadro se visualiza la relación existente entre las regiones detectada y la precision de la detección \ref{Fig: trade-off}.

\begin{figure}[h]
 \centering
  \includegraphics[height=9cm,keepaspectratio=true,clip=true]{imagenes/Evaluacion-exp/presicion-over.png}
  \caption{Trade-Off Average precision-Overlapping}
	\label{Fig: trade-off}
\end{figure}
