...
Existen en la actualidad diversas funciones de costo que nos permite calcular el error del modelo por ejemplo 
...
En la mayoría de las aplicaciones para algoritmos de \ac{ml} abarcan mucho más que solo uno característica, en algunos casos hasta decenas de millones de parámetros. Para encontrar el mínimo valor para grandes dimensiones de datos usamos el algoritmo llamado \textbf{Descenso de Gradiente}(\textit{gradient descent} en ingles). 

El descenso de gradiente es un algoritmo de optimización que busca encontrar el mínimo local o global de una función. Este algoritmo permite que el modelo aprenda el gradiente o la dirección que el modelo debe seguir para reducir el error; en cada iteración gradualmente converge hacia un mínimo optimizando los valores $w_i $ que minimizan 	la función de costo.
\begin{figure}[H] \centering
