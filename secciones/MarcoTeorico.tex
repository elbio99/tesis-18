...
\paragraph{Fuente de energía}: es la radiación electromagnética que capta el sensor, como mencionamos anteriormente puede tratarse de una fuente pasiva o activa.
...
\par \textbf{Resolución Espectral}: la resolución espectral especifica el numero y la anchura de las badas espectrales que puede ser discrimidadas por el sensor.
\begin{figure}[H] \centering
...
\begin{figure}[H]
 \centering
  ...
  \caption{Ejemplo Arboles de decisión}
...
\begin{figure}[H]
 \centering
  \includegraphics[height=8cm,keepaspectratio=true,clip=true]{imagenes/MarcoTeorico/random-forest.png}
  \caption{Ejemplo RF}
...
\begin{enumerate}
\item depth (profundidad): corresponde al numero de filtros usados para la operación de convolución como veremos mas adelante (Sec:\ref{sub:convolucion}). Por ejemplo la primera capa toma la imagen original, luego diferentes neuronas a partir de la profundidad de la red se activa realizando operaciones como puede ser detección de bordes, color, etc
...
\begin{figure}[H]
 \centering
  ...
  \caption{Ejemplo de regiones propuestas.}% \\ (Adaptado de:{http://www.cs.toronto.edu/~byang/})}
