...
En modelos de \ac{ml} debemos encontrar aquellos parámetros que minizan la función de costo como se menciono previamente, este es un poblema de optmización ya que si encontramos la solución podemos encontrar esos parámetros que minimizan el error.

En la mayoría de algoritmos de \ac{ml} deben aprender mas de un parámetro, en algunos casos hasta decenas de millones de parámetros; es por esto que todos los algortimos de \ac{ml} son algoritmos de optmización ya que se busca aprender los parámetros mencionado anteriormente.

Para encontrar el mínimo valor para grandes dimensiones de datos el algoritmo mas utlizado es el llamado \textbf{Descenso de Gradiente} (\textit{gradient descent} en ingles). El descenso de gradiente es un algoritmo de optimización que busca encontrar el mínimo local o global de una función. Este algoritmo permite que el modelo aprenda el gradiente o la dirección que el modelo debe seguir para reducir el error; en cada iteración gradualmente converge hacia un mínimo optimizando los valores $w_i $ que minimizan 	la función de costo.
\begin{figure}[H] \centering
